...
jest.mock('@dhis2/app-runtime', () => ({
  useTimeZoneConversion: () => ({
    fromServerDate: timestamp => {
      ...
      dhis2Date.serverTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
      return dhis2Date;
    }
  })
}));
describe('<NextRun>', () => {
  it('returns the next run time for an enabled job and a future execution time', () => {
    ...
    const wrapper = mount( /*#__PURE__*/React.createElement(NextRun, {
  ...
  it('returns the relative time in tooltip for an enabled job and a future execution time', () => {
    ...
    jest.spyOn(global.Date, 'now').mockImplementation(() => now);
    const wrapper = shallow( /*#__PURE__*/React.createElement(NextRun, {
      nextExecutionTime: future,
      enabled: true
    }));
