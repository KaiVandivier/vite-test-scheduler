...
describe('<Schedule>', () => {
  it('renders a human readable cron for the CRON scheduling type', () => {
    const wrapper = shallow( /*#__PURE__*/React.createElement(Schedule, {
      schedulingType: "CRON",
  ...
  it('renders a delay for the FIXED_DELAY scheduling type', () => {
    ...
    expect(wrapper.text()).toEqual(expect.stringContaining('1000 seconds after last run'));
  });
  it('renders a dash for an unrecognised scheduling type', () => {
    const wrapper = shallow( /*#__PURE__*/React.createElement(Schedule, {
      schedulingType: "NONEXISTENT"
    }));
