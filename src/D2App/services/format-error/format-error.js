...
const formatError = error => {
  var _error$details, _error$details$respon;
  const validationErrors = {};
  const errorReports = (_error$details = error.details) === null || _error$details === void 0 ? void 0 : (_error$details$respon = _error$details.response) === null || _error$details$respon === void 0 ? void 0 : _error$details$respon.errorReports;

  /**
   * Return a generic validation error if there are no errorReports
   */
  if (!(errorReports !== null && errorReports !== void 0 && errorReports.length)) {
    validationErrors[FORM_ERROR] = error.message ? [error.message] : [i18n.t('Something went wrong but no error message was provided')];
